...
* **Silver Layer** (`silver` schema): The cleansed and conformed layer. This step features heavy transformations using Common Table Expressions (CTEs), including:
  ...
  * **Data Quality & Quarantining:** Invalid order records (e.g., missing customer IDs, negative quantities) are flagged with `is_valid = FALSE` and assigned a `quarantine_reason`. Customer records undergo specific checks like email format validation (`is_valid_email`).
...
*(Note: The pipeline supports dual ingestion patterns, allowing you to simulate both direct SQL database transfers and CSV file loads. Because the Bronze ingestion scripts truncate the target tables before loading, running the CSV ingestion scripts immediately after the SQL ones will overwrite the data loaded by the SQL method).*
...
3. `pipeline/raw/export_to_csv.sql` - *Exports the raw tables to CSV (adjust file paths in the script to match your local machine). This ensures you have the files ready for the CSV ingestion option below.*

### 2. Bronze Ingestion
4. `pipeline/bronze/customers_from_sql.sql` - *Ingests customers via direct SQL.*
5. `pipeline/bronze/orders_from_sql.sql` - *Ingests orders via direct SQL.*
6. `pipeline/bronze/customers_from_csv.sql` - *Ingests customers via CSV (adjust file paths to match your local machine).*
7. `pipeline/bronze/orders_from_csv.sql` - *Ingests orders via CSV (adjust file paths to match your local machine).*
...
* If testing the CSV ingestion, ensure the PostgreSQL server has read/write permissions to the directories specified in the `export_to_csv.sql` and `*_from_csv.sql` files.
